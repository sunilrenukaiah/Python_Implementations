...
def replaceTextInFile(fullpath,patternToSearch,TextToReplace):
    ...
    for dirpath, _, filenames in os.walk(fullpath):
        for filename in filenames:
            if filename.endswith(exten.lower()):
                countOfFiles = countOfFiles + 1
                fullpath = os.path.join(dirpath, filename).lower()
                # print("In : ",fullpath)
                try:
...
def readParams():
    global args, regex, repex, srchtxt, reptxt, exten, folderPath, regexFlag, srchtxtFlag,srcFolder,extension
    exten = ""
    regexFlag=0
    srchtxtFlag=0
...
if __name__ == "__main__":
    global args, regex, repex, srchtxt, reptxt, exten, folderPath, regexFlag, srchtxtFlag,extension
